...
HELP
    ...
    conversion_prompt=$(load_prompt "spec-conversion.txt")
    conversion_prompt="${conversion_prompt//__ABS_PLAN_FILE__/$abs_plan_file}"
    conversion_prompt="${conversion_prompt//__ABS_SPEC_DIR__/$abs_spec_dir}"
    conversion_prompt="${conversion_prompt//__FEATURE_NAME__/$feature_name}"
    conversion_prompt="${conversion_prompt//__PLAN_FILE__/$plan_file}"
    conversion_prompt="${conversion_prompt//__PROJECT_TYPE__/$project_type}"
    conversion_prompt="${conversion_prompt//__IS_GREENFIELD__/$is_greenfield}"
    conversion_prompt="${conversion_prompt//__DATE__/$(date '+%Y-%m-%d')}"
...
cmd_implement() {
    if [[ "${1:-}" == "--help" ]] || [[ "${1:-}" == "-h" ]] || [[ "${1:-}" == "help" ]]; then
        cat << 'HELP'
Usage: cr implement [spec-dir] [--json] [--non-interactive] [--model MODEL] [--fallback-model MODEL]
...
SPEC FILE: $abs_spec_dir/SPEC.md"
            ...
            design_review_prompt=$(load_prompt "review-design.txt")
            design_review_prompt="${design_review_prompt//__DESIGN_CONTEXT__/$design_context}"
            design_review_prompt="${design_review_prompt//__DESIGN_URL__/$design_url}"
            design_review_prompt="${design_review_prompt//__REVIEW_SESSION_DIR__/$review_session_dir}"
            design_review_prompt="${design_review_prompt//__DESIGN_TODOS_DIR__/$design_todos_dir}"
...
- [ ] Lint clean"
    ...
    local conversion_prompt
    conversion_prompt=$(load_prompt "fix-conversion.txt")
    conversion_prompt="${conversion_prompt//__SPEC_NAME__/$spec_name}"
    conversion_prompt="${conversion_prompt//__FIX_TYPE_LABEL__/$fix_type_label}"
    conversion_prompt="${conversion_prompt//__ABS_TODO_FILES__/$abs_todo_files}"
    conversion_prompt="${conversion_prompt//__FIX_DIR__/$fix_dir}"
    conversion_prompt="${conversion_prompt//__TODAY_DATE__/$today_date}"
    conversion_prompt="${conversion_prompt//__TODO_COUNT__/$todo_count}"
    conversion_prompt="${conversion_prompt//__PROJECT_TYPE__/$project_type}"
    conversion_prompt="${conversion_prompt//__QUALITY_GATES__/$quality_gates}"
...
"
        ...
        local design_prompt
        design_prompt=$(load_prompt "design-iteration.txt")
        design_prompt="${design_prompt//__ITERATION__/$iteration}"
        design_prompt="${design_prompt//__URL__/$url}"
        design_prompt="${design_prompt//__STATE_FILE__/$state_file}"
        design_prompt="${design_prompt//__SESSION_DIR__/$session_dir}"
        design_prompt="${design_prompt//__PREV_CONTEXT__/$prev_context}"
        validate_prompt "$design_prompt" "design-iteration"
